...
async function getfare(pickup, destination) {
    ...
    if (isNaN(distance) || isNaN(time)) throw new Error('Invalid distance or time from map service');
...
exports.createride = async (req, res, next) => {
    try {
        ...
        if (!fare[vehicletype.toLowerCase()]) return res.status(400).json({ message: 'Invalid vehicle type' });
        ...
        const capitansInRadius = await mapservice.getCapitansInRadius(pickupCoords.lat, pickupCoords.lng, 10);
        ...
        res.status(201).json({ message: 'Ride created successfully', ride: rideWithUser });
    } catch (error) {
        next(error);
    }
};
...
exports.getfare = async (req, res, next) => {
    try {
        ...
        if (!pickup || !destination) return res.status(400).json({ message: 'Origin and destination are required' });
...
exports.confirmride = async (req, res, next) => {
    try {
        ...
        if (!rideid || !capitan) return res.status(400).json({ message: 'rideid and capitan are required' });
...
exports.startride = async (req, res, next) => {
    try {
        ...
        const ride = await ridemodel.findById(rideid).populate('user capitan').select('+otp');
        if (!ride) return res.status(404).json({ message: 'Ride not found' });
        if (ride.status !== 'accepted') return res.status(400).json({ message: 'Ride not accepted or already started' });
        ...
        if (ride.user?.socketId) {
            sendMessageToSocketId(ride.user.socketId, { event: 'start-ride', data: ride });
        }
...
exports.finishride = async (req, res, next) => {
    try {
        ...
        const capitan = req.user; // JWT middleware sets req.user

        const ride = await ridemodel.findOne({ _id: rideid, capitan: capitan._id }).populate('user capitan');
        if (!ride) return res.status(404).json({ message: 'Ride not found' });
        if (ride.status !== 'ongoing') return res.status(400).json({ message: 'Ride is not ongoing' });
