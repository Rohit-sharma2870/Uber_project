...
exports.register = [
  ...
  body("vehiclecolour")
    .isLength({ min: 3 })
    .withMessage("Vehicle colour must have at least 3 characters"),
  body("vehicleplate")
    .isLength({ min: 3 })
    .withMessage("Vehicle number must have at least 3 characters"),
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json({ message: "Captain registered successfully", capitan });
    } catch (err) {
      console.error(err);
      res
        .status(500)
        .json({ message: "Error while creating captain", error: err.message });
    }
  },
];
